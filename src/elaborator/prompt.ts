...
function intentLine(s: Signal): string {
  switch (s.type) {
    ...
    case "dep-changed":
      return `- RECONCILE \`us:${s.target}\` (signal ${s.id}) — ${s.detail ?? "a dependency changed"}; update its Agent Spec to match.`;
  }
}
...
export function buildElaboratorPrompt(parts: PromptParts): string {
  ...
  sections.push(
    [
      ...
      "",
      "Give every AS block an explicit `(id)=` label and link it: `:expands:` for the US id(s) it",
      "details, `:depends-on:` for AS blocks it relies on. Give the desired end-state (by its criteria,",
      "not a hard-coded result), the method to reach it, and verification steps. Use project-relative",
      "paths; never hard-code time/environment-dependent values.",
      "",
      "When done, run `clawloop spec validate` and fix what it reports. If a related block in another",
      "file must change too, claim it with `clawloop signals get us:<id>`. Then report finished targets",
      "with `clawloop signals solved <signal-id>,<signal-id>`.",
    ].join("\n"),
  );
